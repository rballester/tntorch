...
class Tensor(object):
    ...
    def __add__(
        ...
        for n in range(this.dim()):
            ...
            if self.batch:
                c1_st = torch.zeros([core2.shape[0], core2.shape[1], this.shape[n + 1], core1.shape[3]], device=device)
                c2_st = torch.zeros([core1.shape[0], core1.shape[1], this.shape[n + 1], core2.shape[3]], device=device)
            else:
                c1_st = torch.zeros([core2.shape[0], this.shape[n], core1.shape[2]], device=device)
                c2_st = torch.zeros([core1.shape[0], this.shape[n], core2.shape[2]], device=device)
